...
const copyEmbed = () => 
    gulp.src(config.paths.src.embed, { dot: true })
      .pipe(gulp.dest(config.paths.build.embed))
      

const copyStatic = () => 
  gulp.src(config.paths.src.static, { dot: true })
...
const createDir = (cb) => {
  fs.mkdirSync('build/direct-bill-pay');
  cb();
}

const moveFiles = (cb) => {
  fs.rename('build/texas-health.html', 'build/direct-bill-pay/texas-health.html', (err) => {console.log('err', err)});
  fs.rename('build/reliant.html', 'build/direct-bill-pay/reliant.html', (err) => {console.log('err', err)});
  fs.rename('build/energy-texas.html', 'build/direct-bill-pay/energy-texas.html', (err) => {console.log('err', err)});
  fs.rename('build/the-general-insurance.html', 'build/direct-bill-pay/the-general-insurance.html', (err) => {console.log('err', err)});
  cb();
}
...
const watchJs = () => gulp.watch(config.paths.watch.js, gulp.series(compileJs, copyEmbed));
...
const buildStuff = gulp.parallel(compileFonts, compileJs, compileWebp, compileImages, compileCSS, copyStatic, copyEmbed);
...
const organize = gulp.series(createDir);
const build = gulp.series(clean, buildHTML, buildStuff, organize);
